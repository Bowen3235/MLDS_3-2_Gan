...
class Gan(object):
    def __init__(self):
        ...
        self.ge_optimizer = optim.Adam( self.gener.parameters() , lr = 1e-3 ,betas=(0.5, 0.99)  )
        ...
        for i in range( total_epo ):
            ...
            for idx ,( noise , img , tags , w_tags , tags_w_str ) in enumerate( The_Dataloader ):
                    ...
                    if idx % 10 ==0:
                        ...
                        name = "try_new_image/" + "epo_" + str( i ) + "iterater" + str(idx) +"/"
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.bn_text   = nn.BatchNorm1d( 128 , momentum = 0.9 )
    ...
    def forward(self ,text_input ,noise_input):
        noise_input = noise_input.view( noise_input.size(0) , 128 ,1 ,1 )
        x = self.main( noise_input )
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
              ...
              #1024 , 4 , 4
        self.last = nn.Conv2d(1024, 1 , 4, 1, 0)
           

    def forward(self , input_text , pic):
        pic = self.main( pic )
        x  = torch.sigmoid( self.last( pic ).view( pic.size(0) , 1 ) )
        return x
